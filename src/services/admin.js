...
const PRE_REG_URL = process.env.PRE_REGISTRATION_URL;
...
const acceptHacker = async (shellIDs, history) => {
  try {
    ...
    if (String(e).includes("400")) alert("Hacker already accepted");
    else console.log(e);
  }
};
...
const getApplicants = async (page, q, history, filter = "") => {
  try {
    ...
    const { data } = await admin.get(
      `/application?page=${page}&q=${q}&filter=${filter}`,
      config
    );
    ...
    String(e).includes("401") ? console.log(e) : alert("No Hackers found");
  }
};
